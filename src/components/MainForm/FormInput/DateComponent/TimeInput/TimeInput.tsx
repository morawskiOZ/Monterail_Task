...
class TimeInput extends Component<any, any> {
  ...
  lastVal: string;
    ...
    isValid(val) {
        ...
        const isValidMinutes = (minutes) => (Number.isInteger(minutes) && hours >= 0 && hours < 13) || Number.isNaN(minutes);
        ...
        // check mm and HH
        if (valArr[0] && valArr[0].length && (parseInt(valArr[0], 10) < 0 || parseInt(valArr[0], 10) > 13)) {
            return false;
        }
